...
def rot13hardcoded(phrase: str) -> str:
    ...
    key: str = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"
    crypt: str = "nopqrstuvwxyzabcdefghijklmNOPQRSTUVWXYZABCDEFGHIJKLM"
    # with import string: key = string.ascii_letters; crypt = key[13:] + key[:13]
    # but let's limit our self's to pure build-in functions ;)
    transform: dict = dict(zip(key, crypt))
